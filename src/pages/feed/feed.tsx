          ...
          <div>
            ...
            <div className={styles.ListCol}>
              <ul className={styles.infoOrdersListReady}>
                {orders && ordersDoneMax.slice(0, 9).map((item) => (
                  <li key={item._id}>{item.number}</li>
                ))}
              </ul>
              <ul className={styles.infoOrdersListReady}>
                {orders && ordersDoneMax.slice(10, 19).map((item) => (
                  <li key={item._id}>{item.number}</li>
                ))}
              </ul>
            </div>
          ...
          <div>
            ...
            <div className={styles.ListCol}>
              <ul className={styles.infoOrdersList}>
                {orders && ordersNotDoneMax.slice(0, 9).map((item) => (
                  <li key={item._id}>{item.number}</li>
                ))}
              </ul>
              <ul className={styles.infoOrdersList}>
                {orders && ordersNotDoneMax.slice(10, 19).map((item) => (
                  <li key={item._id}>{item.number}</li>
                ))}
              </ul>
            </div>
